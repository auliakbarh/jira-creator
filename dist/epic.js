...
exports.DEFAULT_EPIC_OUTPUT_DIR = void 0;
exports.buildBreakdownPrompt = buildBreakdownPrompt;
exports.generateBreakdown = generateBreakdown;
exports.saveBreakdownPlan = saveBreakdownPlan;
const promises_1 = require("fs/promises");
const path_1 = __importDefault(require("path"));
const gemini_client_1 = require("./gemini-client");
exports.DEFAULT_EPIC_OUTPUT_DIR = 'output-uac';
...
// ─── Prompt builder ───────────────────────────────────────────────────────────
function buildBreakdownPrompt(input, lang) {
...
async function generateBreakdown(input, opts = {}) {
    const text = input.trim();
    if (!text)
        throw new Error('Input kosong — berikan deskripsi epic.');
    const prompt = buildBreakdownPrompt(text, opts.lang ?? 'en');
    const result = await (0, gemini_client_1.generateJSON)(prompt, opts.model, BREAKDOWN_SCHEMA);
...
async function saveBreakdownPlan(breakdown, outDir, timestamp) {
    ...
    const filePath = path_1.default.join(outDir, `epic-${slug}-${timestamp}.json`);
    await (0, promises_1.writeFile)(filePath, JSON.stringify(breakdown, null, 2) + '\n', 'utf-8');
    return filePath;
}
